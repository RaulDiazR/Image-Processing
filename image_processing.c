...
void invertirHorizontalGrises(const char *entrada, const char *salida, FILE *log) {
    ...
    if (dib.bitsPerPixel != 24) {
        ...
        logError(log, "Solo se soportan imagenes de 24 bits.");
        return;
    }
    ...
    if (!row) { fclose(fin); fclose(fout); logError(log, "Memoria insuficiente."); return; }
    ...
    for (int y = 0; y < dib.height; y++) {
        ...
        for (int p = 0; p < padding; p++) fputc(0x00, fout);
    }
